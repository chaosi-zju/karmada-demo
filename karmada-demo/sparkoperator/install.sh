...
while [[ $(kubectl --context karmada-apiserver get MutatingWebhookConfiguration spark-operator-webhook -o yaml | grep caBundle) == "" ]]; do echo "wait MutatingWebhookConfiguration ready.."; sleep 3; done
while [[ $(kubectl --context karmada-apiserver get ValidatingWebhookConfiguration spark-operator-webhook -o yaml | grep caBundle) == "" ]]; do echo "wait ValidatingWebhookConfiguration ready.."; sleep 3; done
...
#kind load docker-image ghcr.io/apache/spark-docker/spark:3.5.2 --name member2
#kind load docker-image ghcr.io/apache/spark-docker/spark:3.5.2 --name member3
#
#
#ka apply -f cpp.yaml
